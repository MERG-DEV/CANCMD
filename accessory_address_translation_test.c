...
main(int, char**)
{
  ...
  const struct test_values test_data[] = {{   0, ASOF, 0x81, 0xF8},
                                          {   0, ASON, 0x81, 0xF9},
                                          {   1, ASOF, 0x81, 0xFA},
                                          {   1, ASON, 0x81, 0xFB},
                                          {   3, ASOF, 0x81, 0xFE},
  ...
  for (size_t test_index = 1; number_of_tests > test_index; ++test_index)
  {
    const struct test_values *test = &test_data[test_index];

    s_ptr = &s_queue[s_head];

    s_ptr->status.valid = 0;
    ...
    printf("Test %lu: Event %s(%u); "
           ...
           "Got: 0x%X, 0x%X; %s\n",
           test_index,
           test->is_acc_on ? "ASON" : "ASOF" , test->event_number,
           test->expected_d0, test->expected_d1,
           s_ptr->d[0], s_ptr->d[1],
